...
export default function QuizSection({ unit, user, progress, quizSubmission, onComplete }) {
  ...
  // }
  if (quizSubmission) {
    const pct = Math.round((quizSubmission.score / questions.length) * 100);
    return (
      <div className="animate-fade-in">
        <div className={`rounded-2xl p-8 text-center mb-6 ${pct >= 70 ? "bg-green-500/10 border border-green-500/30" : "bg-red-500/10 border border-red-500/30"}`}>
          ...
          <p className="text-foreground font-semibold">{quizSubmission.score}/{questions.length} correct</p>
          <p className="text-muted-foreground text-sm mt-1">{pct >= 70 ? "nice job!" : "Try again!"}</p>
        ...
        <div className="space-y-4">
          {questions.map((q, i) => {
            const correct = quizSubmission.answers[i] === q.correct_index;
            return (
                ...
                <div className="space-y-1.5 ml-6">
                  {q.options?.map((opt, oi) => (
                    <div key={oi} className={`text-xs px-3 py-1.5 rounded-lg ${
                      ...
                      oi === quizSubmission.answers[i] && !correct ? "bg-red-500/20 text-red-400" :
